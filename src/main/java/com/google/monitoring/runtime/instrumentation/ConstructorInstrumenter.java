...
/**
 * Instruments bytecode by inserting a specified call in the constructor of a given class. This
 * class is intended to be loaded by a javaagent; end-users will want to add {@link
 * ConstructorCallback}s by invoking {@link #instrumentClass(Class, ConstructorCallback)}.
 */
public class ConstructorInstrumenter implements ClassFileTransformer {
  ...
  /**
   * We have a read-modify-write operation when doing a put in samplerMap (above) and retransforming
   * the class. This lock protects multiple threads from performing that operation concurrently.
   */
  private static final Object samplerPutAtomicityLock = new Object();

  /** Whether to call the samplers when subclasses of the given class are constructed. */
  ...
  /**
   * Given the bytes representing a class, add invocations of the ConstructorCallback method to the
   * constructor.
   *
  ...
  /** The per-method transformations to make. Really only affects the <init> methods. */
  static class ConstructorMethodAdapter extends MethodVisitor {
     ...
     * and the LocalVariablesSorter constructor requires a reference to this adapter. The only
   ...
   * anything if o is a subclass of the class that was supposed to be tracked.
   *
  ...
  /**
   * The class that deals with per-class transformations. Basically, invokes the per-method
   * transformer above if the method is an {@code <init>} method.
   */
